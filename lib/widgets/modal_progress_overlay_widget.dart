...
class ModalRoundedProgressOverlay extends StatefulWidget {
  final double _opacity;
  final String _textMessage; // optional message to show
  final Function(ProgressOverlayHandler handler) _handlerCallback; // callback that will give a handler object to change widget state.

  ModalRoundedProgressOverlay({
    required Function handleCallback,
    String message = "", // some text to show if needed...
    double opacity = 0.7, // opacity default value
  })  : _textMessage = message,
        _opacity = opacity,
        _handlerCallback = handleCallback as dynamic Function(ProgressOverlayHandler);

  @override
  State createState() => _ModalRoundedProgressOverlayState();
}

//StateClass ...
class _ModalRoundedProgressOverlayState extends State<ModalRoundedProgressOverlay> {
  ...
  void initState() {
    ...
    ProgressOverlayHandler handler = ProgressOverlayHandler();
  ...
  Widget build(BuildContext context) {
    ...
    if (!_isShowing) return Container(); 

    // here we return a layout structure that show a roundedProgressBar with a simple text message
    return Material(
